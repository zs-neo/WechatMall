<template>
	<view class="container">
				...
				<view class="w-100 d-flex align-items-center overflow-hidden">
					<scroll-view class="flex-fill overflow-hidden" scroll-x>
						<view class="w-100 d-flex align-items-center">
							...
							<view class="w-100" v-for="(item, index) in cart" :key="index">
		...
		<view class="footer">
			...
			<button type="primary" @click="payFinal()">支付</button>
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			payFinal() {
				...
				//TODO
				for (var foodItem of this.cart) {
					order.orderDetailsList.push({
						foodid: foodItem.fid,
						amount: foodItem.number
					});
				}
				console.log(order);
				uni.request({
					url: 'http://localhost:8000/order/addWechatOrder',
					method: 'POST',
					data: {
						order: order,
						id:this.clientid
